...
beforeEach(() => {
    ...
    jest.spyOn<any, any>(adapter, "getWebsocketConnection").mockImplementation(
        () => Promise.resolve({
...
test("send mock request", async () => {
    jest.spyOn<any, any>(adapter, "getNewPendingRequest").mockImplementation(
        (request: JsonRPCRequest, resolve: Function, reject: Function): PendingRequest => {
            resolve("Hi!")
...
test("handle incoming messages", () => {
    ...
    const messageEventWrongFormat = new MessageEvent("mock", {data: eventRawDataWrongFormat})
    ...
    expect(() => adapter.handleNewMessage(messageEvent.data))
        .toThrow(new Error("Request 123 does not exist in the queue."))

    const rpcRequest = new JsonRPCRequest("testMethod")
    const pendingRequest = new PendingRequest(rpcRequest, (() => "XX"), () => {
        return
    })

    jest.spyOn<any, any>(pendingRequest, "resolve")
    jest.spyOn<any, any>(adapter, "removeFromQueue")
    jest.spyOn<any, any>(adapter, "getFromQueue").mockImplementation(
