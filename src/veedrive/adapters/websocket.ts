...
export default class WebsocketAdapter extends CommunicationAdapterBase {
    ...
    private readonly removeFromQueue = (pendingRequest: PendingRequest): boolean =>
        this.requestQueue.delete(pendingRequest.request.id)

    private readonly getFromQueue = (requestId: string) =>
        this.requestQueue.get(requestId)

    private readonly sendWebsocketMessage = async (method: string, params?: Json, id?: string): Promise<JsonRPCRequest> => {
    ...
    private readonly getNewPendingRequest = (request: JsonRPCRequest, resolve: Function, reject: Function): PendingRequest =>
        new PendingRequest(request, resolve, reject)
    ...
    public readonly handleNewMessage = (eventData: string) => {
        const response: JsonRPCResponse = JsonRPCResponse.createFromString(eventData)
    ...
    public readonly sendRequest = async (method: string, params?: Json, id?: string): Promise<any> =>
